...
import { isTradeBetter } from '~/utilities/isTradeBetter';
    ...
    if (amountSpecified && currencyIn && currencyOut && allowedPairs.length > 0) {
      ...
      for (let i = 1; i <= maxHops; i++) {
        ...
        if (tradeType === TradeType.EXACT_INPUT) {
          const amountIn = amountSpecified;
          currentTrade =
            Trade.bestTradeExactIn(allowedPairs, amountIn, currencyOut, options)[0] ?? null;
        } else {
          const amountOut = amountSpecified;
          currentTrade =
            Trade.bestTradeExactOut(allowedPairs, currencyIn, amountOut, options)[0] ?? null;
        }
