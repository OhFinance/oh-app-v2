...
export default function WithdrawModal({ bank }: { bank: Bank }) {
  const isOpen = useModalOpen(ApplicationModal.WITHDRAW);
  const toggle = useToggleModal(ApplicationModal.WITHDRAW);

  const { currencies, parsedAmounts } = useDerivedStakeInfo(bank);

  const [title, setTitle] = useState(`Withdraw ${currencies.DEPOSIT?.symbol}`);
  ...
  const receivingAmount = useMemo(() => {
    if (parsedAmounts.WITHDRAW && totalSupply && totalValueLocked) {
      return parsedAmounts.WITHDRAW.multiply(totalValueLocked).divide(totalSupply);
    }
    return undefined;
  }, [parsedAmounts.WITHDRAW, totalSupply, totalValueLocked]);
  ...
  const onDeposit = useCallback(async () => {
    let parsedAmount = parsedAmounts.WITHDRAW;
    let currency = currencies.WITHDRAW;
  ...
  return (
    <OhModal title={title} isOpen={isOpen} onDismiss={toggle}>
      <Wrapper>
        {!attemptingTx && !txHash ? (
          <ModalIcon view={ModalView.WITHDRAW} amount={receivingAmount} />
        ) : attemptingTx ? (
          <ModalIcon view={ModalView.WITHDRAWING} amount={receivingAmount} />
        ) : txHash ? (
          <ModalIcon view={ModalView.WITHDRAW_COMPLETE} hash={txHash} />
        ) : (
          <ModalIcon view={ModalView.WITHDRAW} amount={receivingAmount} />
        )}
        {!txHash && (
          <Flex flexDirection="row" justifyContent="space-between" marginTop="8px">
            <Flex flexDirection={'row'} alignItems="flex-end">
              ...
              <Flex flexDirection={'column'}>
                <ThemedText.Main fontSize="14px">Withdraw</ThemedText.Main>
                <ThemedText.Main fontSize="26px">
                  {parsedAmounts.WITHDRAW?.toFixed(3, { groupSeperator: ',' })}
                </ThemedText.Main>
                <Flex alignItems={'center'}>
                  ...
                  />
                  <ThemedText.Main fontSize="16px">{currencies.WITHDRAW?.symbol}</ThemedText.Main>
                </Flex>
                ...
                <Flex alignItems={'center'}>
                  <img
                    ...
                    style={{ marginRight: 3, borderRadius: 16 }}
                  />
                  <ThemedText.Main fontSize="16px">{bank.underlyingToken.symbol}</ThemedText.Main>
                </Flex>
              </Flex>
            </Flex>
